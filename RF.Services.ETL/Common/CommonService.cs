...
namespace RF.Services.ETL.Common
{
    public abstract class CommonService
    {
        public IUnitOfWork UnitOfWork { protected get; set; }
        public IMemoryBus Bus { get; set; }
        ...
        /// <returns></returns>
        /// <exception cref="ETLException">Repository not found for the requested type " + typeof(T).Name</exception>
        internal virtual IReadWriteRepository<T> GetRepositoryByType<T>() where T : class
        {
            ...
            throw new ETLException("Repository not found for the requested type " + typeof(T).Name);
        }
    }
}